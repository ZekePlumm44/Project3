...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    List<char*> *progNames = new List<char*>;
   ...
   // finally, run an initial user program if requested to do so
    ListIterator<char*> *iter = new ListIterator<char*>(progNames);
    if (userProgName != NULL) { 
	while(!(iter->IsDone())) {
		Thread *t = new Thread(iter->Item());
		t->Fork((VoidFunctionPtr) RunUserProg,(void*)iter->Item());
		iter->Next();	
	}
    }
